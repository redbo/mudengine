...
// This is largely a copy of tcell's tscreen.go, modified to work
// without environment variables or an actual tty, so we can have a
// bunch of them in one process.

package headlesstcell
...
func NewScreen(c io.ReadWriter, term string, columns, lines int) (tcell.Screen, error) {
...
func (t *tScreen) Init() error {
	...
	*/
	...
	t.cells.Resize(t.ti.Columns, t.ti.Lines)
	t.cursorx = -1
	t.cursory = -1
	t.resize()
	t.Unlock()
...
func (t *tScreen) prepareKeyMod(key tcell.Key, mod tcell.ModMask, val string) {
	if val != "" {
...
func (t *tScreen) prepareKey(key tcell.Key, val string) {
...
func (t *tScreen) Fill(r rune, style tcell.Style) {
	t.Lock()
	if !t.fini {
...
func (t *tScreen) SetContent(x, y int, mainc rune, combc []rune, style tcell.Style) {
	t.Lock()
	if !t.fini {
...
func (t *tScreen) GetContent(x, y int) (rune, []rune, tcell.Style, int) {
	t.Lock()
...
func (t *tScreen) SetCell(x, y int, style tcell.Style, ch ...rune) {
	if len(ch) > 0 {
...
func (t *tScreen) resize() {
	if t.winW != t.w || t.winH != t.h {
		t.cx = -1
		t.cy = -1

		t.cells.Resize(t.winW, t.winH)
		t.cells.Invalidate()
		t.h = t.winH
		t.w = t.winW
		t.PostEvent(tcell.NewEventResize(t.winW, t.winH))
	}
}
...
func (t *tScreen) PollEvent() tcell.Event {
	select {
	case <-t.quit:
		return nil
	case ev := <-t.evch:
...
func (t *tScreen) PostEventWait(ev tcell.Event) {
	t.evch <- ev
}

func (t *tScreen) PostEvent(ev tcell.Event) error {
	select {
	case t.evch <- ev:
		return nil
	default:
...
func (t *tScreen) parseFunctionKey(buf *bytes.Buffer, evs *[]tcell.Event) (bool, bool) {
	b := buf.Bytes()
	partial := false
...
func (t *tScreen) parseRune(buf *bytes.Buffer, evs *[]tcell.Event) (bool, bool) {
	b := buf.Bytes()
	if b[0] >= ' ' && b[0] <= 0x7F {
...
// Return an array of tcell.Events extracted from the supplied buffer. This is done
...
func (t *tScreen) HasKey(k tcell.Key) bool {
	if k == tcell.KeyRune {
		return true
	}
	return t.keyexist[k]
